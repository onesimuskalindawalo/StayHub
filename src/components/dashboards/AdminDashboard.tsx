...
export function AdminDashboard() {
  ...
  return (
    <div className="max-w-6xl space-y-12">
      <header>
        <h2 className="text-4xl font-bold tracking-tight text-slate-900">System Insights</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <motion.div
            ...
            className="p-8 bg-white rounded-3xl border border-slate-100 transition-all hover:bg-slate-50"
          >
            <div className="flex items-center justify-between mb-4">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">{stat.name}</p>
              <span className="text-[10px] font-bold text-slate-300 bg-slate-100 px-2 py-0.5 rounded-full">{stat.change}</span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-stretch">
        <div className="lg:col-span-8 bg-white p-8 rounded-3xl border border-slate-100">
            ...
            {revenueData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={revenueData}>
                  ...
                  />
                  <Bar dataKey="revenue" fill="currentColor" className="text-slate-900" radius={[6, 6, 0, 0]} barSize={40} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
        ...
        <div className="lg:col-span-4 bg-white p-8 rounded-3xl border border-slate-100 flex flex-col">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <QuickLink icon={Hotel} title="Hostels" href="/dashboard/hostels" />
        <QuickLink icon={DoorOpen} title="Rooms" href="/dashboard/rooms" />
        <QuickLink icon={FileText} title="Reports" href="/dashboard/reports" />
      </div>
    </div>
  );
}
...
function QuickLink({ icon: Icon, title, href }: { icon: any, title: string, href: string }) {
  return (
    <motion.button
      whileHover={{ y: -4 }}
      className="group flex items-center justify-between p-8 bg-white border border-slate-100 rounded-3xl hover:bg-slate-50 transition-all"
    >
      <div className="flex items-center space-x-4">
        <div className="p-3 rounded-2xl bg-slate-50 group-hover:bg-white group-hover:text-black transition-colors">
