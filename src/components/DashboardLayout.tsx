...
const sidebarItems: SidebarItem[] = [
  { name: 'Dashboard', href: '/', icon: LayoutDashboard, roles: [UserRole.STUDENT, UserRole.ADMIN, UserRole.WARDEN] },
  { name: 'Hostels', href: '/hostels', icon: Hotel, roles: [UserRole.ADMIN, UserRole.WARDEN, UserRole.STUDENT] },
  { name: 'Rooms', href: '/rooms', icon: DoorOpen, roles: [UserRole.ADMIN, UserRole.WARDEN] },
  { name: 'My Bookings', href: '/bookings', icon: BookOpen, roles: [UserRole.STUDENT] },
  { name: 'All Bookings', href: '/admin/bookings', icon: BookOpen, roles: [UserRole.ADMIN, UserRole.WARDEN] },
  { name: 'Payments', href: '/payments', icon: CreditCard, roles: [UserRole.STUDENT, UserRole.ADMIN] },
  { name: 'Users', href: '/users', icon: Users, roles: [UserRole.ADMIN] },
  { name: 'Profile', href: '/profile', icon: UserIcon, roles: [UserRole.STUDENT, UserRole.ADMIN, UserRole.WARDEN] },
];
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-slate-50 transition-all duration-500">
        ...
        <nav className="flex-1 px-4 mt-8 space-y-1">
          {filteredItems.map((item) => (
            ...
            >
              <item.icon className={cn("w-4 h-4", location.pathname === item.href ? "text-black" : "text-slate-300")} />
